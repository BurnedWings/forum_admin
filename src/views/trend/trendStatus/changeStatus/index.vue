<template>
  <div class="audit-container">
    ...
    <el-table :data="tableData" class="status-table" style="width: 100%;">
      ...
      <el-table-column prop="image.length" label="配图张数" width="130"></el-table-column>
      ...
      <el-table-column prop="user.username" label="作者昵称" width="160"></el-table-column>
      <el-table-column class="doColumn" label="操作" width="200">
        <template slot-scope="scope">
          <el-button
            ...
            type="primary"
          >查看详情</el-button>
          <el-button @click="toLockTheTrend(scope.row._id)" slot="reference" size="mini" type="danger">锁定</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-drawer
...
export default {
  ...
  methods: {
    ...
    async lockTheTrend() {
      ...
      const ret = await this.$API.trend.lockTheTrend(this.targetLockTrend,this.textarea.trim())
      if(ret.code===200){
        ...
        this.targetLockTrend = null;
        this.dialogVisible = false;
        this.getAuditTrend(this.currentPage)
        this.$message({
          type:'success',
          message:'该动态已锁定~'
        })
    ...
    //查询已过审动态
    async getAuditTrend(page) {
      let ret;
      if (page) {
        ret = await this.$API.trend.getAuditTrend(page);
      } else {
        ret = await this.$API.trend.getAuditTrend();
      }
      if (ret.code === 200) {
    ...
    async handleCurrentChange(val) {
      ...
      const ret = await this.$API.trend.getAuditTrend(val);
      if (ret.code === 200) {
        this.tableData = ret.trendList;
        this.totalCount = ret.totalCount;
      }
    ...
    async getCurrentTrend(){
      const ret = await this.$API.trend.getCurrentTrend(this.$route.params.detailTrend)
      if (ret.code === 200) {
        if (ret.trendList[0]) {
          this.tableData = ret.trendList;
        }else{
  ...
  mounted() {
    if (!this.$route.params.detailTrend) {
       this.getAuditTrend();
    } else {
