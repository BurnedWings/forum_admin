<template>
  <div class="app-container">
    ...
    <el-table :data="tableData" class="status-table" style="width: 100%;">
      ...
      <el-table-column prop="title" label="评论内容" width="180">
      ...
      <el-table-column prop="date" label="评论发布日期" width="150">
        <template slot-scope="scope">
          <span
      ...
      <el-table-column prop="bio" label="用户反馈" width="240">
        <template slot-scope="scope">
          <el-popover
            placement="top-start"
            width="200"
      ...
      <el-table-column class="doColumn" label="操作" width="180">
        <template slot-scope="scope">
          <el-popconfirm
            @confirm="deleteTheComment(scope.row.comment,scope.row.reply)"
            title="你确定要删除该评论嘛"
          >
...
export default {
  ...
  methods: {
    ...
    //删除评论
    async deleteTheComment(comment, reply) {
      let ret;
      if (comment) {
        //删除评论
        const myComment = comment._id;
        const trend = comment.trend;
        ret = await this.$API.trend.deleteTrendComment(myComment, trend);
      } else {
        //删除回复
        ret = await this.$API.trend.deleteTrendReply(reply._id);
      }
      if (ret.code === 200) {
        if (this.tableData.length === 1 && this.currentPage != 1) {
          this.currentPage -= 1;
        }
        this.getTrendCommentReportMessage(this.currentPage)
        this.$message({
          type: "success",
          message: "删除成功，任务已完成~"
        });
    ...
    async finishCommentReport(taskId) {
      const ret = await this.$API.trend.finishTrendCommentReport(taskId);
      if (ret.code === 200) {
        if (this.tableData.length === 1 && this.currentPage != 1) {
          this.currentPage -= 1;
        }
    ...
    //获取动态评论举报信息
    async getTrendCommentReportMessage(page) {
      let ret;
      if (page) {
        ret = await this.$API.trend.getTrendCommentReportMessage(page);
      } else {
        ret = await this.$API.trend.getTrendCommentReportMessage();
      }
      if (ret.code === 200) {
        this.tableData = ret.reportList;
    ...
    async handleCurrentChange(val) {
      ...
      this.getTrendCommentReportMessage(val);
    }
  },
  mounted() {
    this.getTrendCommentReportMessage();
  }
};
</script>
